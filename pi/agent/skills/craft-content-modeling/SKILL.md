...
description: "Craft CMS 5 content modeling — sections, entry types, fields, Matrix, relations, project config, content architecture. Covers section types, entry/field layout design, field types, Matrix/nested entries, relations, eager loading (.with()/.eagerly()), multi-site propagation, taxonomy, asset volumes, image transforms, user groups, permissions, entrify, CKEditor vs Matrix, multi-language, Formie forms. Triggers on: sections, entry types, fields, Matrix, relatedTo, eager loading, categories, tags, globals, preloadSingles, propagation, multi-site, project config, YAML, content strategy, Formie deployment. Always use for content architecture, sections/fields, Matrix, relations. NOT for PHP (craftcms), front-end Twig (craft-site), or buildchain."
---
