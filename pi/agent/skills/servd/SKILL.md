---
name: servd
description: "Servd (servd.host) — Craft-specialised managed hosting for Craft CMS. Covers git push-to-deploy, servd.yaml, local→staging→production with uni-directional Project Config sync, servd/craft-asset-storage plugin (S3 Flysystem on svdcdn.com CDN, off-server transforms, Imager-X/ImageOptimize), Servd static caching (full/tag-based purge, {% dynamicInclude %}, CSRF injection) with Blitz reverse-proxy, MariaDB/MySQL via SSH, auto/manual backups, Dedicated Queue Runner, env vars/secrets, ephemeral filesystem (Redis + remote volumes), plugin constraints, Servd vs Craft Cloud. Triggers on: servd.yaml, servd/craft-asset-storage, SERVD_* vars, files.svdcdn.com, Servd caching, {% dynamicInclude %}, servd-asset-storage/clone, pull-database, push-assets, clear-caches, Dedicated Queue Runner, deploy to Servd. NOT for Craft Cloud (craft-cloud), Forge/bare metal (craftcms/deployment.md), or DDEV (ddev)."
---
