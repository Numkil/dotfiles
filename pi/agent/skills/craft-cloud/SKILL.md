---
name: craft-cloud
description: "Craft Cloud — Pixel & Tonic's serverless hosting for Craft CMS. Covers craft-cloud.yaml, Build→Migrate→Release pipeline, craftcms/cloud package, edge image transforms via Cloudflare, edge static caching with cache.rules+ESI, Cloud-managed S3 filesystem, MySQL 8/Postgres 15 DBs, console command runner, scheduled cron, auto queue jobs, custom domains/SSL, preview environments, ephemeral filesystem, plugin Cloud compatibility, self-hosted→Cloud migration. Triggers on: craft-cloud.yaml, craftcms/cloud, cloud.esi(), php craft cloud/up, App::isEphemeral(), CRAFT_EPHEMERAL, edge.craft.cloud, Cloud filesystem, ESI islands, deploy/migrate to Craft Cloud. NOT for Servd (servd), Forge/bare metal (craftcms/deployment.md), or DDEV local dev."
---
