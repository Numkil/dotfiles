...
description: "Craft CMS 5 PHP coding standards. ALWAYS load for any PHP in Craft plugins/modules, and when running ECS/PHPStan or ddev craft make. Covers PHPDoc blocks (@author, @since, @throws), section headers, class organization, naming conventions, defineRules/validation, beforePrepare()/addSelect(), MemoizableArray, DateTimeHelper vs Carbon, strict_types, ?string notation, typed properties, void returns, early returns, match over switch, CP Twig conventions, form macros, Craft::t translations, ECS/PHPStan config. Triggers on: service classes, models, controllers, elements, element queries, records, queue jobs, migrations, PHP code review, requireAdmin vs requirePermission, kebab-case permission handles, App::env() over getenv(), App::parseEnv() for $VAR, no em-dash in user copy. NOT for front-end Twig (craft-twig-guidelines) or CP JS (craft-garnish). Required for Craft PHP."
---
