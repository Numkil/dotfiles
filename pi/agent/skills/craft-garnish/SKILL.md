---
name: craft-garnish
description: "Garnish — Craft CMS's built-in JS UI toolkit for the control panel. ALWAYS load for CP JavaScript: plugin CP assets, custom field type JS, element index JS, CP webpack config, garnishjs/window.Garnish code. Covers Garnish.Base.extend, init, setSettings, addListener, on/off/trigger, destroy, Modal, HUD, DisclosureMenu, CustomSelect, ContextMenu, Select, BaseDrag, DragSort, DragDrop, focus management, ARIA helpers, keyboard navigation, Craft.CP, Craft.Slideout, Craft.ElementEditor, Craft.sendActionRequest, CP memory issues, event cleanup. Triggers on: Garnish patterns, CP JS, Craft.* references, focus traps, slideout lifecycle. NOT for front-end JS (Alpine, Vue, htmx) or Twig (craft-site)."
---
